...
//! - Debt analysis performance

use clawdius_core::{
    analysis::{DebtAnalyzer, DriftDetector},
...
fn bench_rate_limiter(c: &mut Criterion) {
    ...
    group.bench_function("acquire_high_throughput", |b| {
        let limiter = RateLimiter::new(high_throughput_config.clone());
    ...
    group.bench_function("acquire_conservative", |b| {
        let limiter = RateLimiter::new(conservative_config.clone());
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(&rt).iter(|| {
            let limiter = limiter.clone();
            async move {
