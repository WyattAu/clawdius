...
impl TimelineRepository for SqliteBackend {
    ...
    async fn rollback(&self, _checkpoint_id: &CheckpointId) -> Result<()> {
        // Rollback requires filesystem access which depends on workspace_root.
        // The full implementation will be added when TimelineStore is migrated.
        // For now, database-side rollback (deleting versions after checkpoint) works.
        Ok(())
    }
    ...
    async fn preview_rollback(&self, checkpoint_id: &CheckpointId) -> Result<RollbackPreview> {
        ...
        Ok(RollbackPreview {
            ...
            files_to_restore: Vec::new(), // Populated from file_versions
    ...
    async fn export_checkpoint(&self, checkpoint_id: &CheckpointId) -> Result<ExportedCheckpoint> {
        ...
        // Load file versions for this checkpoint
        let versions = self.with_conn(|conn| {
            ...
            let files = stmt
                .query_map(params![checkpoint_id.0.clone()], |row| {
                    Ok(ExportedFile {
                        path: PathBuf::from(row.get::<_, String>(0)?),
                        content: String::new(), // Content stored separately on disk
                        is_binary: false,
                        hash: row.get(1)?,
                    })
                })
    ...
    async fn cleanup_snapshots(&self) -> Result<usize> {
        // No filesystem snapshot cleanup in database-only mode
        Ok(0)
    }
