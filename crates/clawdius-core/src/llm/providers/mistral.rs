//! Mistral provider
//!
//! Uses genai's native Mistral adapter which auto-detects models starting with
//! `mistral` and routes to `https://api.mistral.ai/v1/`.
//!
//! Supports `mistral-large-latest`, `mistral-medium-latest`, `mistral-small-latest`,
//! `codestral-latest`, and other Mistral AI models.
//!
    ...
    async fn chat_stream(&self, messages: Vec<ClawdiusMessage>) -> Result<mpsc::Receiver<String>> {
        ...
        tokio::spawn(async move {
            match client.exec_chat_stream(&model, chat_req, None).await {
                Ok(stream_response) => {
                    ...
                    while let Some(result) = stream.next().await {
                        match result {
                            ...
                            Err(e) => {
                                tracing::error!("Mistral stream error for model {}: {}", model, e);
                                break;
                            },
                        }
                    }
                ...
                Err(e) => {
                    tracing::error!("Mistral stream init error for model {}: {}", model, e);
                    drop(tx);
                },
            }
        });
...
mod tests {
    ...
    fn test_mistral_provider_new() {
        ...
        assert_eq!(provider.unwrap().model, "mistral-large-latest");
    }

    #[test]
    fn test_mistral_provider_custom_model() {
        let provider = MistralProvider::new("test-key", Some("codestral-latest"));
        assert!(provider.is_ok());
        assert_eq!(provider.unwrap().model, "codestral-latest");
    }

    #[test]
    fn test_mistral_provider_small() {
        let provider = MistralProvider::new("test-key", Some("mistral-small-latest"));
        assert!(provider.is_ok());
        assert_eq!(provider.unwrap().model, "mistral-small-latest");
    }
