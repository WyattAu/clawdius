...
impl SymbolIndex {
    ...
    pub fn index_document(&mut self, uri: &Url, text: &str) {
    ...
    pub fn hover(&self, uri: &Url, line: u32, character: u32) -> Option<HoverInfo> {
        ...
        for sym in &doc.indexed {
            if sym.line == line && character <= sym.end_character {
...
impl HoverInfo {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        if self.references > 0 {
            let plural = if self.references > 1 { "s" } else { "" };
            let _ = write!(md, " ({} reference{plural})", self.references);
        }
        md.push('\n');
...
class MyClass(Base):
    pass
";
        let uri = Url::parse("file:///test.py").expect("valid uri");
...
";
        let uri = Url::parse("file:///test.go").expect("valid uri");
        let (symbols, _) = extract_symbols_with_context(&uri, code);
        assert_eq!(symbols.len(), 2);
...
class Calculator {}
";
        let uri = Url::parse("file:///test.ts").expect("valid uri");
        let (symbols, _) = extract_symbols_with_context(&uri, code);
    ...
    fn test_index_document() {
        ...
        let uri = Url::parse("file:///test.rs").expect("valid uri");
        index.index_document(&uri, "fn main() {}");
        let symbols = index.document_symbols(&uri).expect("should have symbols");
    ...
    fn test_hover_at_symbol() {
        ...
        let info = hover.expect("hover should be Some");
    ...
    fn test_goto_definition_same_doc() {
        ...
        assert!(def.is_some());
        assert_eq!(def.expect("def should be Some").name, "foo");
    }

    #[test]
    fn test_references_across_docs() {
        let mut index = SymbolIndex::new();
        let uri1 = Url::parse("file:///a.rs").expect("valid uri");
        let uri2 = Url::parse("file:///b.rs").expect("valid uri");
        index.index_document(&uri1, "fn shared() {}");
        index.index_document(&uri2, "fn shared() {}");
